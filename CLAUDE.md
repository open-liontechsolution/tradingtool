...
| `strategies/rsi_reversion.py` | Connors-style RSI mean-reversion (high win-rate by design): buys short-RSI dips only while price is above a trend SMA, exits the bounce via a small profit target. Validated defaults (`trend_sma_n=200`, `rsi_period=3`, `rsi_entry_long=20`, `exit_mode=target`, `target_pct=0.05`, `stop_pct=0.15`) deliver ≥65% win-rate (≈75%) and net-positive returns on BTC/ETH/SOL across 4h and 1d. Long-only by default; the wide stop is a backstop, not the primary exit, so drawdowns are larger than the breakout family |
| `strategies/trend_rider.py` | Leveraged momentum trend-rider (asymmetric payoff — the OPPOSITE profile to `rsi_reversion`): Donchian-breakout entry gated by a **rising-SMA regime filter** (long only when `close > SMA(regime_sma_n)` and the SMA is rising) + ADX, ATR initial stop, ATR chandelier trailing, and a regime-flip exit. Low win-rate, but a few trends carry the year and bear years stay ~flat (regime filter keeps it out). Long-only by default. Validated defaults (`donchian_n=20`, `regime_sma_n=100`, `regime_slope_n=40`, `adx_threshold=15`, `atr_stop_mult=3`, `atr_trail_mult=8`) at **leverage 3** turned 1000→10,243 (10.24×) on SOLUSDT 1d in calendar-2023 with no liquidation (maxDD −28%). High variance: occasional deep losing years are expected; meant for the "maximise upside" mandate, not capital preservation. The 3×ATR stop keeps leveraged losses below the isolated-margin liquidation line at lev ≤ ~5 |
...
- **Risk-based position sizing (#144)**: optional per-config sizing mode that derives the entry's notional from the stop distance instead of always deploying `current_portfolio × leverage`. Selected via `signal_configs.position_sizing_mode ∈ {'full_equity', 'risk_based'}` (default `'full_equity'`, exact regression with pre-#144 behaviour). In `risk_based`, both engines compute `risk_amount = current_portfolio × max_loss_per_trade_pct`, `qty = risk_amount / abs(entry_price - stop_base)`, `target_notional = qty × entry_price`, then clip to `max_notional = current_portfolio × max(leverage, 1)`. When `target_notional > max_notional` the entry still opens but at the leverage cap, and the trade is flagged `sim_trades.sizing_clipped=1` (frontend shows a "CLIP" chip on the SimTrades row) — this is the failure mode where realised loss-if-stopped *exceeds* the target risk %, and is exactly when #142's skip toggle remains useful as a safety net (the two settings compose orthogonally). Reuses `max_loss_per_trade_pct` as the single "% of equity per trade" input — no second field. `entry_price` is the **trigger candle's close** in both engines (parity with #142's choice; deterministic at signal-emission time, even in `open_next` where fill_price ≠ trigger_close). Shared helper: `backend.risk.compute_risk_based_size` returning `(invested, qty, sizing_clipped)`; raises `ValueError` on degenerate input (`entry_price ≤ 0`, `current_portfolio ≤ 0`, `distance == 0`) — live catches and skips, backtest never hits the path because strategies don't emit such signals. Live persists `invested_amount` + `sizing_clipped` on `sim_trades` at signal-creation time; the fill quantity is recomputed downstream as `invested / fill_price`. `liquidation_price` is unchanged — depends only on `(side, entry_price, leverage, mm)`, not on sizing. PATCH allows switching modes on a live config; in-flight trades are unaffected, only future entries see the new mode. Backtest reads the mode from `params['position_sizing_mode']`; live reads it from `signal_configs.position_sizing_mode`; both must be wired in lockstep for a parity case (see `tests/integration/test_parity.py::test_parity_risk_based_slot_a`).
