...
- **`paths-ignore` + required status checks = catch-22.** The `develop-merge-protection` ruleset requires `Lint backend / Lint frontend / Test backend / Build frontend` to pass. If `ci.yml`'s `paths-ignore` is broad enough that a PR touches ONLY ignored paths, CI skips entirely → required checks never report → mergeStateStatus=BLOCKED. Workaround on a single PR: include any non-ignored file (a small CLAUDE.md note often qualifies as a useful doc edit). Structural fix done in #92: removed `.github/workflows/**` from `paths-ignore` so workflow-only PRs now trigger CI naturally. Still ignored: `helm/env/**` (auto-bot tag bumps don't need CI re-run).
