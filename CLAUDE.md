...
- **Secrets**: never committed. `helm/env/secrets.example.yaml` is the template; copy to `helm/env/secrets-<env>.yaml` (gitignored — both `secrets.yaml` and `secrets-*.yaml` are covered) and apply with `kubectl apply -f`.
