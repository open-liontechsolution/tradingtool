...
- **Test matrix** (two functions in `test_parity.py`):
  - `test_parity_slot_strategy` — unleveraged (`leverage=1.0`) over `slot_name × strategy_name`. Strategies: `breakout`, `breakout_trailing`, `support_resistance`, `support_resistance_trailing` (×4 slots = 16 cases).
  - `test_parity_leveraged_slot_d` — slot_d × strategy with `leverage=10`, `maintenance_margin_pct=0.005`. Exercises liquidation parity (#58 Gap 1): both engines close at `liquidation_price` with `exit_reason='liquidated'` when the candle's low/high crosses it, and stop opening new entries from that point onward (live: `status='blown'`; backtest: local `blown` flag).
  - All cases use `modo_ejecucion=close_current` — the only mode with full engine parity post-#49 (`open_next` has a residual exit-fill gap, deferred to #58 Gap 2).
...
- **What's NOT compared yet**:
  ...
  - **`open_next` exit-fill semantic**: backtest exits at the *current* candle's open while live exits at close — same trade list, different exit prices. Tracked in #58 Gap 2.
