...
- **`paths-ignore` + required status checks = catch-22.** `ci.yml` has `paths-ignore: [".github/workflows/**", "helm/env/**"]`, and the `develop-merge-protection` ruleset requires `Lint backend / Lint frontend / Test backend / Build frontend` to pass. A PR touching ONLY those ignored paths skips CI entirely → the required checks never report → mergeStateStatus=BLOCKED. Workaround: include any non-ignored file in the PR (a small CLAUDE.md note often qualifies as a useful doc edit). The structural fix is to drop `paths-ignore` from `ci.yml` and let CI run on every PR — but doing that itself runs into the same catch-22, so it has to ride along with another change.
